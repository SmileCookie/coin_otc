package com.world.model.balaccount.job.finaccdownload;
...
import com.world.model.balaccount.job.finaccdownload.thread.FinAccDownloadThread;
...
/**
 * <p>标题: 提现记录对账查询</p>
 * <p>描述: 提现记录对账查询从支付中心同步数据</p>
 * <p>版权: Copyright (c) 2017</p>
 * @author flym
 * @version 
 */
public class FinAccDownloadWork extends Worker {
	/**
	 * 
	 */
...
//	private String sql = "";
	...
	public FinAccDownloadWork(String name, String des) {
		super(name, des);
	}
	...
	public void run() {
		try {
			...
			log.info("FinAccDownloadWork...strNowTime = " + strNowTime + ", tsTodayTime = " + tsTodayTime + ", lastUpdateTime = " + lastUpdateTime);
...
//            sysLogDao.addSysLog("admin", logId, logId, "adminMonitor", "FinAccDownloadWork", 2, logContent , "admin", "admin", "");
			...
			while(iteCoinProps.hasNext()) {
				...
				if(fundType > 0) {
					/*创建线程,按每个币种进行处理查询*/
					FinAccDownloadThread finAccDownloadThread = new FinAccDownloadThread(fundType, fundTypeName);
//						finAccDetailsThread.run();
					finAccDownloadWorkPool.execute(finAccDownloadThread);
				} else {
					log.info("没有此币种,无法同步!...");
				}
			}
			...
			finAccDownloadWorkPool.shutdown();
			
			while (true) {
				/*如果关闭后所有任务都已完成，则返回 true。注意，除非首先调用 shutdown 或 shutdownNow，否则 isTerminated 永不为 true。*/
				if(finAccDownloadWorkPool.isTerminated()) {
					break;
				} else {
					log.info("提现记录对账进行中...");
			...
			logContent = "提现记录对账查询结束!!!【核算耗时：" + (endTime - startTime) + "】";
			log.info(logContent);
//			sysLogDao.addSysLog("admin", logId, logId, "adminMonitor", "FinAccDownloadWork", 2, logContent, "admin", "admin", "");
//			}
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		FinAccDownloadWork finAccDownloadWork = new FinAccDownloadWork("FinAccDownloadWork", "提现记录对账查询");
		finAccDownloadWork.run();
	}
