...
public class PayUserBean extends Bean{
	...
	private BigDecimal freez = BigDecimal.ZERO;
    //活动冻结金额
...
//	private BigDecimal overdue = BigDecimal.ZERO;//滞纳金
	...
	private BigDecimal rate = BigDecimal.ZERO;//利率
	private int riskManage;//风险控制类型

	private BigDecimal arrearsLx = BigDecimal.ZERO;//拖欠利息
	private BigDecimal hasLx = BigDecimal.ZERO;	//已还利息

	/**
	 * 日收益，到账收益，累计收益布放入缓存
	 * start by xwz 2017-06-09
	 */
	private BigDecimal  earningsOfDay = BigDecimal.ZERO;	//日收益
	private BigDecimal  arrivedOfDay = BigDecimal.ZERO; 	//到账收益
	private BigDecimal  totalEarnings = BigDecimal.ZERO;	//累计收益

	private BigDecimal surplusLoan = BigDecimal.ZERO;	//剩余投资额度（=投资限额-成功投资额-等待投资额）
	private BigDecimal totalLoan = BigDecimal.ZERO;//投资总额（待投入+已投入）
...
//	private BigDecimal fees;//用户放贷手续费比例
	...
	/*start by xzhang 20170906 增加后台报表展示字段*/
	private BigDecimal entrustFreeze = BigDecimal.ZERO;//委托冻结金额

	public BigDecimal getEntrustFreeze() {
		return freez.subtract(withdrawFreeze).subtract(outWait);
	}
	/*end*/
...
//	}
	...
	public BigDecimal getEarningsOfDay() {
		return earningsOfDay;
	}

	public void setEarningsOfDay(BigDecimal earningsOfDay) {
		this.earningsOfDay = earningsOfDay;
	}

	public BigDecimal getArrivedOfDay() {
		return arrivedOfDay;
	}

	public void setArrivedOfDay(BigDecimal arrivedOfDay) {
		this.arrivedOfDay = arrivedOfDay;
	}
	...
	public BigDecimal getSurplusLoan() {
		return surplusLoan;
	}

	public void setSurplusLoan(BigDecimal surplusLoan) {
		this.surplusLoan = surplusLoan;
	}
