...
@FunctionAction(jspPath = "/admins/trade/stat/", des = "盘口交易")
public class Index extends FinanAction {
    ...
    public void index() {
        ...
        query1.setSql("select unitPrice avgPrice, (numbers-completeNumber) numbers,userId,types isBuy " +
...
//        Query<Guadan> query1 = guadanDao.getQuery();
//        query1.setSql("select * from (select sum(totalMoney)/sum(numbers)/100 as avgPrice, sum(numbers-completeNumber)/100000000.0 as numbers ,userId,types isBuy" +
...
//        List<Guadan> sellList = guadanDao.find();
        ...
        query2.setSql("select * from transrecord where unitPrice>0 and status>1 order by times desc limit 100");
