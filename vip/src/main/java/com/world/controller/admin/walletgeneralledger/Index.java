package com.world.controller.admin.walletgeneralledger;
 ...
 * @author flym
 */
@FunctionAction(jspPath = "/admins/walletgeneralledger/", des = "钱包总账报表")
...
//				+ "from finAccWalletBill " + sqlWhere + " group by fundsType) fma on fa.fundsType = fma.fundsType";
            ...
            sql = "select fa.fundsType fundsType, abs(f1a.1amount) amount1, abs(f1a.1fee) fee1, abs(f2a.2amount) amount2, abs(f2a.2fee) fee2, " + "abs(f3a.3amount) amount3, abs(f3a.3fee) fee3, abs(f4a.4amount) amount4, abs(f4a.4fee) fee4, " + "abs(f5a.5amount) amount5, abs(f5a.5fee) fee5, abs(f6a.6amount) amount6, abs(f6a.6fee) fee6, " + "abs(f7a.7amount) amount7, abs(f7a.7fee) fee7, abs(f8a.8amount) amount8, abs(f8a.8fee) fee8, " + "abs(f37a.sumSameFee37) sumSameFee37, abs(f3a2.sumSameFee3) sumSameFee3, abs(f7a2.sumSameFee7) sumSameFee7, " + "fma.maxHeight, fma.minHeight, fma.maxTime, fma.minTime " + "from (select distinct fundsType from finAccWalletBill " + sqlWhere + ") fa " + "left join (select fundsType, sum(txAmount) 1amount, sum(fee) 1fee from finAccWalletBill " + "where dealType = 1 " + sqlAnd + " group by fundsType ) f1a on fa.fundsType = f1a.fundsType " + "left join (select fundsType, sum(txAmount) 2amount, sum(fee) 2fee from finAccWalletBill " + "where dealType = 2 " + sqlAnd + " group by fundsType ) f2a on fa.fundsType = f2a.fundsType " + "left join (select fundsType, sum(txAmount) 3amount, sum(fee) 3fee from finAccWalletBill " + "where dealType = 3 " + sqlAnd + " group by fundsType ) f3a on fa.fundsType = f3a.fundsType " + "left join (select fundsType, sum(txAmount) 4amount, sum(fee) 4fee from finAccWalletBill " + "where dealType = 4 " + sqlAnd + " group by fundsType ) f4a on fa.fundsType = f4a.fundsType "
                    //xzhang 2017.08.21 新增四种类型
                    + "left join (select fundsType, sum(txAmount) 5amount, sum(fee) 5fee from finAccWalletBill " + "where dealType = 5 " + sqlAnd + " group by fundsType ) f5a on fa.fundsType = f5a.fundsType " + "left join (select fundsType, sum(txAmount) 6amount, sum(fee) 6fee from finAccWalletBill " + "where dealType = 6 " + sqlAnd + " group by fundsType ) f6a on fa.fundsType = f6a.fundsType " + "left join (select fundsType, sum(txAmount) 7amount, sum(fee) 7fee from finAccWalletBill " + "where dealType = 7 " + sqlAnd + " group by fundsType ) f7a on fa.fundsType = f7a.fundsType " + "left join (select fundsType, sum(txAmount) 8amount, sum(fee) 8fee from finAccWalletBill " + "where dealType = 8 " + sqlAnd + " group by fundsType ) f8a on fa.fundsType = f8a.fundsType "
				/*start by xwz 20171014*/
                    //类型为3，7时多计手续费
                    + "left join (select ff37a.fundsType, sum(ff37a.37sameFee * (ff37a.37cnt - 1) / ff37a.37cnt) sumSameFee37 from ( " + "select fundsType, txid, sum(fee) 37sameFee, count(id) 37cnt from finAccWalletBill " + "where dealType in (3, 7) and txid in (select txid from finAccWalletBill where dealType in (3, 7) " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff37a) f37a on fa.fundsType = f37a.fundsType "
                    //类型为3时多计手续费
                    + "left join (select ff3a.fundsType, sum(ff3a.3sameFee * (ff3a.3cnt - 1) / ff3a.3cnt) sumSameFee3 from ( " + "select fundsType, txid, sum(fee) 3sameFee, count(id) 3cnt from finAccWalletBill " + "where dealType = 3 and txid in (select txid from finAccWalletBill where dealType = 3 " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff3a) f3a2 on fa.fundsType = f3a2.fundsType "
                    //类型为7时多计手续费
                    + "left join (select ff7a.fundsType, sum(ff7a.7sameFee * (ff7a.7cnt - 1) / ff7a.7cnt) sumSameFee7 from ( " + "select fundsType, txid, sum(fee) 7sameFee, count(id) 7cnt from finAccWalletBill " + "where dealType = 7 and txid in (select txid from finAccWalletBill where dealType = 7 " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff7a) f7a2 on fa.fundsType = f7a2.fundsType "
				/*end*/ + "left join (select fundsType, max(blockHeight) maxHeight, min(blockHeight) minHeight, max(configTime) maxTime, min(configTime) minTime " + "from finAccWalletBill " + sqlWhere + " group by fundsType) fma on fa.fundsType = fma.fundsType";
...
//				}
                ...
                Date end = null;
                if (endTime == null) {
                    end = TimeUtil.getSpecifiedDayBefore(new Date());
                }else{
                    end = new Date(endTime.getTime());
                }
                if(startTime !=null){
                    begin = TimeUtil.getSpecifiedDayBefore(new Date(startTime.getTime()));
                }
                List<Reconciliation> reconciliationList=reconciliationDao.getSubList(begin, end, fundsType);
                ...
                for (FinAccWalDailyAccBean finAccWalDailyAccBean : listFinAccWalDailyAcc) {
                    for (Reconciliation reconciliation : reconciliationList) {
                        if (finAccWalDailyAccBean.getFundsType() == reconciliation.getFundsType()) {
                            finAccWalDailyAccBean.setTransactionPlatformAmount(reconciliation.getBookBalance().add(reconciliation.getSysDeduction()).add(reconciliation.getTransactionFee()).add(reconciliation.getWithdrawFee()).subtract(reconciliation.getSysRecharge()).add(reconciliation.getIcoExchange()).subtract(reconciliation.getInternalAdjustmentPositive()).add(reconciliation.getInternalAdjustmentNegative()).add(reconciliation.getExternalAdjustmentPositive()).subtract(reconciliation.getExternalAdjustmentNegative()));
                            finAccWalDailyAccBean.setInternalAdjustmentPositive(reconciliation.getInternalAdjustmentPositive());
                            finAccWalDailyAccBean.setInternalAdjustmentNegative(reconciliation.getInternalAdjustmentNegative());
                            finAccWalDailyAccBean.setExternalAdjustmentPositive(reconciliation.getExternalAdjustmentPositive());
                            finAccWalDailyAccBean.setExternalAdjustmentNegative(reconciliation.getExternalAdjustmentNegative());
                        }
                    }
                }
                ...
                SimpleDateFormat smf = new SimpleDateFormat("yyyy-MM-dd");
                setAttr("queryDate", smf.format(end));
            }
    ...
    public void exportFinAccWalDailyAcc() {
        ...
        try {
            ...
            //xzhang 2017.08.21 新增四种类型
            String[] column = {"fundTypeName", "amount1", "amount2", "hotDetailWalOcAmount", "fee4", "coldWalOcAmount", "amount7", "fee7", "fee3", "hotDownloadWalOcAmount", "fee2", "amount6", "amount5", "amount8", "fee8", "heightRange", "timeRange"};
            String[] tabHead = {"资金类型", "用户充值", "用户提现", "热冲发生额", "热冲到冷网络费", "冷钱包发生额", "冷到其他发生额", "冷到其他网络费", "冷到热提网络费", "热提发生额", "热提到用户网络费", "其他到冷发生额", "其他到热提发生额", "热提到其他发生额", "热提到其他网络费", "区块高度", "对账日期"};
...
//				+ "from finAccWalletBill " + sqlWhere + " group by fundsType) fma on fa.fundsType = fma.fundsType";
        ...
        sql = "select fa.fundsType fundsType, abs(f1a.1amount) amount1, abs(f1a.1fee) fee1, abs(f2a.2amount) amount2, abs(f2a.2fee) fee2, " + "abs(f3a.3amount) amount3, abs(f3a.3fee)-ifnull(abs(f3a2.sumSameFee3),0) fee3, abs(f4a.4amount) amount4, abs(f4a.4fee) fee4, " + "abs(f5a.5amount) amount5, abs(f5a.5fee) fee5, abs(f6a.6amount) amount6, abs(f6a.6fee) fee6, " + "abs(f7a.7amount) amount7, abs(f7a.7fee)-ifnull(abs(f7a2.sumSameFee7),0)fee7, abs(f8a.8amount) amount8, abs(f8a.8fee) fee8, " + "(ifnull(abs(f4a.4amount),0) - ifnull(abs(f3a.3amount),0) - ifnull(abs(f3a.3fee),0) + ifnull(abs(f6a.6amount),0) - ifnull(abs(f7a.7amount),0) - ifnull(abs(f7a.7fee),0) + ifnull(abs(f37a.sumSameFee37),0)) coldWalOcAmount, " + "(ifnull(abs(f1a.1amount),0) - ifnull(abs(f4a.4amount),0) - ifnull(abs(f4a.4fee),0)) hotDetailWalOcAmount, " + "(ifnull(abs(f3a.3amount),0) - ifnull(abs(f2a.2amount),0) - ifnull(abs(f2a.2fee),0) + ifnull(abs(f5a.5amount),0) - ifnull(abs(f8a.8amount),0) - ifnull(abs(f8a.8fee),0)) hotDownloadWalOcAmount, " + "fma.maxHeight, fma.minHeight, fma.maxTime, fma.minTime " + "from (select distinct fundsType from finAccWalletBill " + sqlWhere + ") fa " + "left join (select fundsType, sum(txAmount) 1amount, sum(fee) 1fee from finAccWalletBill " + "where dealType = 1 " + sqlAnd + " group by fundsType ) f1a on fa.fundsType = f1a.fundsType " + "left join (select fundsType, sum(txAmount) 2amount, sum(fee) 2fee from finAccWalletBill " + "where dealType = 2 " + sqlAnd + " group by fundsType ) f2a on fa.fundsType = f2a.fundsType " + "left join (select fundsType, sum(txAmount) 3amount, sum(fee) 3fee from finAccWalletBill " + "where dealType = 3 " + sqlAnd + " group by fundsType ) f3a on fa.fundsType = f3a.fundsType " + "left join (select fundsType, sum(txAmount) 4amount, sum(fee) 4fee from finAccWalletBill " + "where dealType = 4 " + sqlAnd + " group by fundsType ) f4a on fa.fundsType = f4a.fundsType " + "left join (select fundsType, sum(txAmount) 5amount, sum(fee) 5fee from finAccWalletBill " + "where dealType = 5 " + sqlAnd + " group by fundsType ) f5a on fa.fundsType = f5a.fundsType " + "left join (select fundsType, sum(txAmount) 6amount, sum(fee) 6fee from finAccWalletBill " + "where dealType = 6 " + sqlAnd + " group by fundsType ) f6a on fa.fundsType = f6a.fundsType " + "left join (select fundsType, sum(txAmount) 7amount, sum(fee) 7fee from finAccWalletBill " + "where dealType = 7 " + sqlAnd + " group by fundsType ) f7a on fa.fundsType = f7a.fundsType " + "left join (select fundsType, sum(txAmount) 8amount, sum(fee) 8fee from finAccWalletBill " + "where dealType = 8 " + sqlAnd + " group by fundsType ) f8a on fa.fundsType = f8a.fundsType "

				/*start by xwz 20171014*/ + "left join (select ff37a.fundsType, sum(ff37a.37sameFee * (ff37a.37cnt - 1) / ff37a.37cnt) sumSameFee37 from ( " + "select fundsType, txid, sum(fee) 37sameFee, count(id) 37cnt from finAccWalletBill " + "where dealType in (3, 7) and txid in (select txid from finAccWalletBill where dealType in (3, 7) " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff37a) f37a on fa.fundsType = f37a.fundsType " + "left join (select ff3a.fundsType, sum(ff3a.3sameFee * (ff3a.3cnt - 1) / ff3a.3cnt) sumSameFee3 from ( " + "select fundsType, txid, sum(fee) 3sameFee, count(id) 3cnt from finAccWalletBill " + "where dealType = 3 and txid in (select txid from finAccWalletBill where dealType = 3 " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff3a) f3a2 on fa.fundsType = f3a2.fundsType " + "left join (select ff7a.fundsType, sum(ff7a.7sameFee * (ff7a.7cnt - 1) / ff7a.7cnt) sumSameFee7 from ( " + "select fundsType, txid, sum(fee) 7sameFee, count(id) 7cnt from finAccWalletBill " + "where dealType = 7 and txid in (select txid from finAccWalletBill where dealType = 7 " + sqlAnd + " group by txid having count(id) > 1 ) " + ") ff7a) f7a2 on fa.fundsType = f7a2.fundsType "
				/*end*/

                + "left join (select fundsType, max(blockHeight) maxHeight, min(blockHeight) minHeight, max(configTime) maxTime, min(configTime) minTime " + "from finAccWalletBill " + sqlWhere + " group by fundsType) fma on fa.fundsType = fma.fundsType";
