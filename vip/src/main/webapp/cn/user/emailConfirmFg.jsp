						...
						<c:otherwise>
							<div class="form-group">
								<c:if test="true">
									<c:choose>
										<c:when test="${lan eq 'cn'}">
											<p class="text-p">将在<span class="text-primary " id="autoJump">3</span>秒后跳转至忘记密码。</p>
											<a class="text-primary-a" href="${vip_domain }/bw/fgPwdOne" target="_self">立即跳转</a>
										</c:when>
										<c:when test="${lan eq 'hk'}">
											<p class="text-p">將在<span class="text-primary" id="autoJump">3</span>秒後跳轉至忘记密碼。</p>
											<a class="text-primary-a" href="${vip_domain }/bw/fgPwdOne" target="_self">立即跳轉</a>
										</c:when>
										<c:when test="${lan eq 'jp'}">
											<p class="text-p"><span class="text-primary" id="autoJump">3</span>秒後、パスワードを忘れたというページへ。</p>
											<a class="text-primary-a" href="${vip_domain }/bw/fgPwdOne" target="_self">今すぐジャンプします</a>
										</c:when>
										<c:when test="${lan eq 'kr'}">
											<p class="text-p"><span class="text-primary" id="autoJump">3</span>초 후에 비밀번호를 잊다의 페이지로 옯깁니다.</p>
											<a class="text-primary-a" href="${vip_domain }/bw/fgPwdOne" target="_self">당장 이동</a>
										</c:when>
										<c:otherwise>
											<p class="text-p">Jump to forget password after<span class="text-primary" id="autoJump">3</span> seconds. </p>
											<a class="text-primary-a" href="${vip_domain }/bw/fgPwdOne" target="_self">Jump now</a>
										</c:otherwise>
									</c:choose>
								</c:if>
							</div>
        ...
        $(function(){
            ...
            var inter = setInterval(function(){
                ...
                if (t <= 1) {
                    ...
                    <c:choose>
                    <c:when test="${type eq '1' }">
                    window.top.location.href = DOMAIN_VIP + "/bw/fgPwdOne";
                    </c:when>
                    <c:otherwise>
                    window.top.location.href = DOMAIN_VIP + "/bw/fgPwdOne";
                    </c:otherwise>
