	...
	<c:if test="${needAuth}">
		<div class="vip-tip">
			<dl>
				...
				<dd>${L:l(lan,'您还没开启Google认证，请先开启<a target="_blank" style="color:#3366CC" href="/manage/auth/google?oper=0&dealType=googleAuth&dealVal=99">Google认证</a>')}</dd>
			</dl>
		</div>
	</c:if>
	<c:if test="${!needAuth}">
		...
		<div class="fill-form">
			...
			<div class="fill-form-fd">
				...
				<a href="javascript:save();" class="btn btn-primary btn-lg btn-block">
					<c:if test="${oper == 1}">${L:l(lan,'开启提现Google验证')}</c:if>
					<c:if test="${oper == 0}">${L:l(lan,'关闭提现Google验证')}</c:if>
				</a>
			</div>
		</div>
	</c:if>
...
function save(){
	...
	if(mobileCode.length < 6){
		JuaBox.sure("${L:l(lan,'请输入短信验证码')}");
		return;
	}
	if(googleCode.length < 6){
		JuaBox.sure("${L:l(lan,'请输入Google验证码')}");
		return;
	}
	$.ajax({
		type : "POST",
		url : "/manage/auth/changePayGoogleAuth",
		data : {
