						...
						<c:when test="${fn:length(dataList)>0}">
							<c:forEach items="${dataList}" var="bill">
								<tr>
									...
									<td>
										<c:if test="${bill.status==0 }">${L:l(lan,"等待确认") }</c:if>
										<c:if test="${bill.status==1 }">${L:l(lan,"失败") }</c:if>
										<c:if test="${bill.status==2 }">${L:l(lan,"成功") }</c:if>
										<c:if test="${bill.status==3 }">${L:l(lan,"取消") }</c:if>
									</td>
								</tr>
							</c:forEach>
						</c:when>
