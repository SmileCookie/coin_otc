...
function ok(){
	var actionUrl = "/admin/user/modifyCustomerType";
	vip.ajax( {
...
<body>
	<div id="add_or_update" class="main-bd">
			<div class="form-line" style = "display:block;width:100%;"}>
				<span style="float:left;" class="formtit">用户类型：</span>
				<span style="float:left;margin: 2px 20px 0 0px;" class="formcon">
					<select name="customerType" id="customerType" style="width:110px;display: none;">
						<c:forEach items="${map}" var="entry">
							<option style="width:100px;" value=${entry.key} <c:if test="${entry.key==user.customerType}">selected="selected"</c:if>>${entry.value}</option>
						</c:forEach>
					 </select>
				</span>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>
