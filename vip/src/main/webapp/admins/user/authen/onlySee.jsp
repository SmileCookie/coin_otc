...
<div class="main">
    ...
    <div>审核时间：<fmt:formatDate value="${au.checkTime}" pattern="yyyy-MM-dd HH:mm:ss"/></div>
    ...
    <div>
        ...
        <div>审核状态:
            <c:if test="${au.status == 6}"> 通过</c:if>
            <c:if test="${au.status == 7}"> 不通过
                <div>原因:
                    <c:if test="${au.reason == 8}"> 图像经过处理</c:if>
                    <c:if test="${au.reason == 9}"> 图像不清晰</c:if>
                    <c:if test="${au.reason == 10}"> 证件图像类型不符</c:if>
                    <c:if test="${au.reason == 11}"> 平台仅支持满16周岁的用户进行交易</c:if>
                </div>
            </c:if>
        </div>
    </div>
