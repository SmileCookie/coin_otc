				...
				<tr class="item_list_bd item_list_bgcolor">
					...
					<td class="commodity_action br_color">
						<c:if test="${list.isDelete==0 }">
							<c:if test="${list.status == 0 }"><%--手动确认充值成功 --%>
								<a href="javascript:confirm(${list.detailsId });" style="color:red">手动确认</a>
							</c:if>
							<c:if test="${list.type==1 && list.status == 2 }">
								<c:if test="${list.sucConfirm==0 }"><a style="color: green;" href="javascript:succonfirm(${list.detailsId })">账务录入</a></c:if>
								<c:if test="${list.sucConfirm==1 }">-</c:if>
							</c:if>
							<a href="javascript:syncConfirmTimes(${list.detailsId });">同步确认次数</a>
						</c:if>
					</td>
				</tr>
			</tbody>
