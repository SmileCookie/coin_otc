...
<div id="editor-container" class="container">
    <div id="simple">
    <div >活动描述简体：</div>
    <div id="editor-trigger" name ="eventContentCN" id="eventContentCN">
        <p>${eventInfo.eventContentCN }</p>
    </div>
    <div >活动规则描述简体：</div>
    <div id="editor-trigger1" name ="eventRuleCN"  id="eventRuleCN">
        <p>${eventInfo.eventRuleCN}</p>
    </div>
    </div>

</div>
<div id="editor-container1" class="container">
    <div id="simple">
        <div >活动描述繁体：</div>
        <div id="editor-trigger2" name ="eventContentHK" id="eventContentHK">
            <p>${eventInfo.eventContentHK }</p>
        </div>
        <div >活动规则描述繁体：</div>
        <div id="editor-trigger3" name ="eventRuleHK" id="eventRuleHK">
            <p>${eventInfo.eventRuleHK  }</p>
        </div>
    </div>
</div>
<div id="editor-container2" class="container">
    <div id="simple">
        <div >活动描述英文：</div>
        <div id="editor-trigger4" name ="eventContentEN" id="eventContentEN">
            <p>${eventInfo.eventContentEN }</p>
        </div>
        <div >活动规则描述英文：</div>
        <div id="editor-trigger5" name ="eventRuleEN" id="eventRuleEN">
            <p>${eventInfo.eventRuleEN }</p>
        </div>
    </div>
</div>
...
//                uptoken : 'WD3JdgM7tywIZpjeceiYrUoNVJD2l1SzZdXH3ahp:64oKb0chJdhgZnt3-dquzQBG5nY=:eyJzY29wZSI6InVwbG9hZDIiLCJkZWFkbGluZSI6MTQ5NTUyNTU4MH0=',
    ...
    editor1.config.customUpload = true;  // 设置自定义上传的开关
    editor1.config.customUploadInit = uploadInit;  // 配置自定义上传初始化事件，uploadInit方法在上面定义了
    editor1.create();
    var editor1 = new wangEditor('editor-trigger2');
    editor1.config.customUpload = true;  // 设置自定义上传的开关
    editor1.config.customUploadInit = uploadInit;  // 配置自定义上传初始化事件，uploadInit方法在上面定义了
    editor1.create();
    var editor1 = new wangEditor('editor-trigger3');
    editor1.config.customUpload = true;  // 设置自定义上传的开关
    editor1.config.customUploadInit = uploadInit;  // 配置自定义上传初始化事件，uploadInit方法在上面定义了
    editor1.create();
    var editor1 = new wangEditor('editor-trigger4');
    editor1.config.customUpload = true;  // 设置自定义上传的开关
    editor1.config.customUploadInit = uploadInit;  // 配置自定义上传初始化事件，uploadInit方法在上面定义了
    editor1.create();
    var editor1 = new wangEditor('editor-trigger5');
    editor1.config.customUpload = true;  // 设置自定义上传的开关
    editor1.config.customUploadInit = uploadInit;  // 配置自定义上传初始化事件，uploadInit方法在上面定义了
    editor1.create();
</script>
