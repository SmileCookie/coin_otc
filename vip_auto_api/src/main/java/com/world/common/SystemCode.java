...
public enum SystemCode implements SysEnum{
	...
	code_1004(1004, "资金安全密码锁定", "Transaction Password Locked"),
	code_1005(1005, "资金安全密码错误", "Transaction Password Error"),
	code_1006(1006, "实名认证等待审核或审核不通过", "Real-name verification is pending approval or not approval."),
	code_1009(1009, "此接口维护中", "This interface is in maintaining."),
	
	code_2001(2001 , "账户余额不足" , "Insufficient CNY Balance"), 
	...
	code_3006(3006 , "无效的IP或与绑定的IP不一致", "Invalid IP or Differ From the Bound IP"),
	code_3007(3007 , "请求时间已失效", "Invalid Request Time"),
	code_3008(3008 , "交易记录没有找到" , "Not Found Transaction Record"),
	
	code_4001(4001 , "API接口被锁定一个小时或未启用", "API Interface is locked or not enabled."),
	...
	public static SystemCode getSystemCode(int key){
		switch(key){
			...
			case 138: { 
				SystemCode code = SystemCode.code_1001;
				code.setClassName("委托失败-重复提交");
				code.setValue("委托失败-重复提交");
				return code;
			}
			case 200: {
				return SystemCode.code_1000;
