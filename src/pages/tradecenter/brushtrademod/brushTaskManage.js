...
export default class BrushTaskManage extends React.Component {
    ...
    requestTable(){
        axios.get(DOMAIN_VIP+"/brush/task/list").then(res => {
    ...
    modalGoogleCodeBtn(value){
        ...
        axios.post(DOMAIN_VIP+"/common/checkGoogleCode",qs.stringify({
            ...
            if(result.code == 0){
                ...
                }else if(type == 'preserStart'){
                    this.setState({
                        googVisibal:false
                    },()=>this.preserStartTaskBtn(market))
                }else if(type == 'preserStop'){
                    this.setState({
                        googVisibal:false
                    },()=>this.preserStopTaskBtn(market))
                }
                // else{
                //     this.setState({
                //         googVisibal:false
    ...
    startBrushTaskBtn(market){
        ...
        return new Promise((resolve, reject) => {
            axios.get(DOMAIN_VIP+'/brush/task/start',{params:{
                market:market
            }}).then(res => {
                const result = res.data;
                if(result.code == 0){
    ...
    stopBrushTaskBtn(market){
        let self = this
        return new Promise((resolve, reject) => {
            axios.get(DOMAIN_VIP+'/brush/task/stop',{
                params:{
                    market:market
                }
            }).then(res => {
    ...
    allStartTaskBtn(){
        axios.get(DOMAIN_VIP+"/brush/task/allStart").then((res) => {
            const result = res.data
            if(result.code == 0){
                this.requestTable()
            }
    ...
    allStopTaskBtn(){
        axios.get(DOMAIN_VIP+"/brush/task/allStop").then((res) => {
            const result = res.data
            if(result.code == 0){
                this.requestTable()
            }
    ...
    preserStartTask = () => {
        ...
        Modal.confirm({
            ...
            cancelText: '取消',
            onOk() {
                self.modalGoogleCode("preser","preserStart")
            },
            onCancel() {
                console.log('Cancel');
            },
    ...
    preserStartTaskBtn = () => {
        axios.get(DOMAIN_VIP+"/brush/hedge/start").then((res) => {
            const result = res.data
            if(result.code == 0){
                this.requestTable()
            }
    ...
    preserStopTask = () => {
        ...
        Modal.confirm({
            ...
            cancelText: '取消',
            onOk() {
                self.modalGoogleCode("preser","preserStop")
            },
            onCancel() {
                console.log('Cancel');
            },
    ...
    preserStopTaskBtn = () => {
        axios.get(DOMAIN_VIP+"/brush/hedge/stop").then((res) => {
            const result = res.data
            if(result.code == 0){
                this.requestTable()
            }
                        ...
                        <div className="x_panel">
                            <div className="x_content">
                                <div className="col-md-12 col-sm-12 col-xs-12 marbot10">
                                    <div className="right">
                                        <Button type="primary" onClick={this.preserStartTask} >保值开启</Button>
                                        <Button type="danger" onClick={this.preserStopTask} >保值停止</Button>
                                    ...
                                    <table border='1' className="table table-striped jambo_table bulk_action table-linehei">
                                        <thead>
                                            <tr className="headings">
                                                ...
                                                <th className="column-title">当前参考平台</th>
