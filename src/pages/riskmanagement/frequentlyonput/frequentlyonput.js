...
class Connectedaccout extends Component{
    ...
    async componentDidMount(){
        await this.requestTable();
        this.setState({
            limitBtn: pageLimit('coinLargeAccount', this.props.permissList)
        });
    }
    componentDidUpdate(){
    ...
    batchMarking = () => {
        ...
        Modal.confirm({
            ...
            cancelText: '取消',
            onOk() {
                axios.post(DOMAIN_VIP + '/coinLargeAccount/updateall', qs.stringify({idlist:self.state.RowKeys.toString()})).then(res => {
                    const result = res.data;
                    if (result.code == 0) {
                        self.setState({
                            RowKeys: []
    ...
    render(){
        const {time,pageSize,showHide,tableSource,pagination,googVisibal,check, conversionS, conversionE,userid,visible,title,width,pageIndex,RowKeys,limitBtn} = this.state
        return (
            <div className='right-con '>
                 <div className="page-title">
                    当前位置：风控管理 > 币币交易业务 > 大额账户监控
                                    ...
                                    <Table dataSource={tableSource} bordered pagination={{...pagination,current:pageIndex}} locale={{emptyText:'暂无数据'}}
                                     onChange={this.sorter}
                                        ...
                                        <Column title='操作'  dataIndex='id'key='id' render = {(parameter,obj)=>{
                                             return (
                                                <div>
                                                   {obj.state==0 && (limitBtn.indexOf('update')>-1)?<a href='javascript:void(0);' onClick={()=> this.requestState(parameter,1)}>标记</a>:''}
                                                </div>
                                                    )
                                            }}/>
                                    </Table>
