...
import { fetchTransInfo } from '../../../redux/modules/transDetail'
...
const tableHeader = [
    ...
    },
    {
        ...
        name: <FormattedMessage id="收益冻结" />,
        sortName: 'freeze6'
    }, 
    {
        name: <FormattedMessage id="保险投资冻结" />,
        sortName: 'insureInvestFreezeAmount'
    },
    // {
    //     name: <FormattedMessage id="保险奖励冻结" />,
    //     sortName: 'insureRewardFreezeAmount'
    // },
    {
...
class Balance extends React.Component {
        ...
        try{
            ...
            if (result) {
                for (let key in result) {
                    ...
                    var insureInvestFreezeAmount = funds.insureInvestFreezeAmount;
                    //var insureRewardFreezeAmount = funds.insureRewardFreezeAmount;
                    ...
                    record[i].freeze6 = new BigNumber(profit).toFixed(8);  //冻结资金
                    ...
                    record[i].insureInvestFreezeAmount = new BigNumber(insureInvestFreezeAmount).toFixed(8); //保险投资冻结
                    //record[i].insureRewardFreezeAmount = new BigNumber(insureRewardFreezeAmount).toFixed(8); //保险奖励冻结
                    i++;
                }
            }
    ...
    detailRefresh(fundsType) {
        this.setState({ Mstr: <Transfer closeModal={this.modal.closeModal} fromtype={5} totype={1} fundsType={fundsType} /> })
    }
    ...
    render() {
        ...
        //console.log(this.props.coinList.isloaded,'ddddddddddddddd');
            ...
            <div className="bk-assets">
               <div className="cont-row">
                    ...
                    <div className="bbyh-moneyHeader">
                        <h2 className="assets-title unChooseTitle" onClick={() =>browserHistory.push(formatURL('/manage/account/currency'))}><FormattedMessage id="币币账户"/></h2>
                        ...
                        <h2 className="assets-title chooseTitle" onClick={() =>browserHistory.push(formatURL('/manage/account/lcurrency'))}><FormattedMessage id="理财账户"/></h2>
                    </div>
                    ...
                    <div className="assets-detail-right">
                        <div className="assets-detail-value">
                            ...
                            <div className="val-con" style={{"lineHeight":"20px"}}>
                                <i id="cerrencyUnit">{this.props.moneylogo.logo}</i> <span id="cerrencyTotal">{totalAll}</span> / <em id="totalBtc">{totalBtc ? new BigNumber(totalBtc).toFixed(8) : '0.0'}</em> BTC
                            </div>
                    ...
                    <div className="history_list_right">
                        <Link to="/bw/manage/account/fbchargeDownHistory?type=2&fl=0"><div className="history_list"><FormattedMessage id="历史记录" /></div></Link>
                    </div>
                </div>
                                ...
                                coinList.length > 0 && coinList.map((item, index) => {
                                    return (
                                        ...
                                        <tr key={item.stag} >
                                            ...
                                            <td className="text-right">{item.insureInvestFreezeAmount}</td>
                                            {/* <td className="text-right">{item.insureRewardFreezeAmount}</td> */}
...
const mapStateToProps = (state, ownProps) => {
    return {
        coinList: state.transDetail.detail,
