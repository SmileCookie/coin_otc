...
import { doCkMobile } from '../../redux/modules/usercenter'; 
...
@Form
class CkTel extends React.Component{
    constructor(props){
        super(props);
    ...
    cm(){
        if(!this.hasError(this.dictionaries)){
            this.props.doCkMobile(this.state, this.callError, 'smscode')
        }
    }
                ...
                <form className="uauth_wp">
                    <ul className="list">
                        ...
                        <li className={`lst3x ${esmscode[0] && 'err'}`}>
                            ...
                            <div className="plv">
                                <input autoComplete="off" type="text" className="i1" placeholder={formatMessage({id: "请输入短信验证码（水印）"})} name="smscode" value={smscode} onPaste={setSmsCode} onChange={setSmsCode} onFocus={fIn} onBlur={bOut} />
