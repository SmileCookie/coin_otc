/**
 * @author oliver
 * @description  用户与用户成交分析
 */

import Decorator from 'DTPath'
import CommonTable from 'CTable'
import { PAGEINDEX, SHOW_TIME_DEFAULT, TIME_PLACEHOLDER, DAYFORMAT } from 'Conf'
...
@Decorator()
export default class UserAndUserTransactionAnalysis extends React.Component {
    constructor(props) {
        super(props)
        this.defaultState = {
            time:[moment(),moment()],
    ...
    requestTable = async (currIndex, currSize) => {
        const { pageIndex, pageSize,market,time } = this.state
        let params = {
            market,
        ...
        const result = await this.request({ url: '/tradeAnalysis/userTrade', type: 'post' }, params)
        this.setState({
    ...
    createColumns = () => {
        ...
        return [
            ...
            { title: '用户交易数量', dataIndex: 'coinNum',  },
            { title: '用户交易总额', dataIndex: 'totalAmount', render:t => toThousands(t,true) },
            { title: '用户交易均价', dataIndex: 'avgPrice' },
            { title: '交易人数', dataIndex: 'userNum' },
