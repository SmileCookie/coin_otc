...
@Decorator()
export default class UserAndBrushBuyAnalysis extends React.Component {
    constructor(props) {
        super(props)
        this.defaultState = {
            time:[moment(),moment()],
    ...
    requestTable = async (currIndex, currSize) => {
        ...
        const result = await this.request({ url: '/tradeAnalysis/brushBuy', type: 'post' }, params)
        this.setState({
    ...
    createColumns = () => {
        ...
        return [
            ...
            { title: '用户买入数量', dataIndex: 'coinNum', },
            { title: '用户买入总额', dataIndex: 'totalAmount',  render: t => toThousands(t,true) },
            { title: '用户买入均价', dataIndex: 'avgPrice', render: t => toThousands(t,true) },
            { title: '交易人数（买入）', dataIndex: 'userNum' },
