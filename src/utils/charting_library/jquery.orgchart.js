...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    if ($.type(data) === 'object') {
      ...
      .done(function(data, textStatus, jqXHR) {
        if(data.datas.status!=500){
          buildHierarchy($chart, opts.ajaxURL ? data : attachRel(data.datas, '00'), 0, opts);
        }else{
          $(".orgchart").append("<div class='tesz'>"+g[lng]+"</div>")
        }
      })
  ...
  function createNode(nodeData, level, opts) {
   ...
   var title2=nodeData.title2!=undefined ?  nodeData.title2:'' ;
    var $nodeDiv = $('<div onclick="show(this)" class='+nodeData.isRec+' data-name='+nodeData.userName + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? '  id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      .addClass('node ' + (nodeData.className || '') +  (level >= opts.depth ? ' slide-up' : ''))
      .append('<div class="title">' + nodeData[opts.nodeTitle] + '</div>')
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (nodeData[opts.nodeContent] || '') +"</br>"+ title2 +'</div>': '')
