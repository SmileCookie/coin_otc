...
import $ from 'jquery'
import { DOMAIN_TRANS,TIMER } from '../../conf'
...
var HistoryProvider = /** @class */ (function () {
    ...
    HistoryProvider.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate) {
        ...
        var requestParams = {
            ...
            type: resolution,
            since: 1e3 * rangeStartDate,
            to: 1e3 * rangeEndDate,
        };
        var timezoneOffet = new Date().getTimezoneOffset();
        return new Promise(function (resolve, reject) {
            axios.get(_this._datafeedUrl,{params:requestParams}).then((res)=>{
                    ...
                    if (response.s === 'no_data') {
                        ...
                        for (var i = 0; i < response.t.length; ++i) {
                            var time = timezoneOffet > 0?response.t[i]-Math.abs(timezoneOffet)*60*1000:response.t[i]+Math.abs(timezoneOffet)*60*1000;
                            var barValue = {
                                time: time,
...
var DataPulseProvider = /** @class */ (function () {
    function DataPulseProvider(historyProvider, updateFrequency,market) {
        this._subscribers = {};
        this._requestsPending = 0;
        this._historyProvider = historyProvider;
        if(Object.keys(TIMER).length && Object.keys(TIMER).includes(market)){
            var prveTime = Object.values(TIMER[market]);
            for(var i=0;i<prveTime.length;i++){
                clearInterval(prveTime[i])
            }
        }
        TIMER[market] = {}
        TIMER[market]["timer1"] = setInterval(this._updateData.bind(this), updateFrequency);
    }
...
var QuotesPulseProvider = /** @class */ (function () {
    function QuotesPulseProvider(quotesProvider,market) {
        this._subscribers = {};
        this._requestsPending = 0;
        this._quotesProvider = quotesProvider;
        TIMER[market]["timer2"] = setInterval(this._updateQuotes.bind(this, 1 /* Fast */), 10000 /* Fast */);
        TIMER[market]["timer3"] = setInterval(this._updateQuotes.bind(this, 0 /* General */), 60000 /* General */);
    }
...
var UDFCompatibleDatafeedBase = /** @class */ (function () {
    function UDFCompatibleDatafeedBase(datafeedURL, quotesProvider, requester, updateFrequency,market) {
        ...
        this._quotesProvider = quotesProvider;
        this._dataPulseProvider = new DataPulseProvider(this._historyProvider, updateFrequency,market);
        this._quotesPulseProvider = new QuotesPulseProvider(this._quotesProvider,market);
        ...
        if (!this._configuration.supports_group_request) {
            new Promise(function(resolve, reject){
                var objParams = {
                    ...
                    "symbol": symbolName,
                    "exchange": "btcwinex",
                    "exchange-traded": "btcwinex",
                    "exchange-listed": "btcwinex",
                    "timezone": "UTC",
                    "pricescale": 2,
                    "minmov": 1,
                    "minmove2": 0,
                    "has_intraday": true,
                    "intraday_multipliers": ["1", "5", "15", "30", "60", "240", "D","5D","7D", "30D"],
                    "has_daily": true,
                ...
                if(_this._datafeedURL.includes('futures')){
                    resolve(objParams)
                }else{
                    try{
                        $.getJSON(DOMAIN_TRANS + "/getMarket?callback=?&type="+symbolName,function(result) {
                            if (result.isSuc && result.datas.length > 0) {
                                ...
                                objParams = Object.assign(objParams,{
    ...
    UDFCompatibleDatafeedBase.prototype._requestConfiguration = function () {
        ...
        return new Promise(function (resolve, reject) {
            resolve({
                "exchanges": [
                    {
                        "value": "btcwinex",
                        "name": "btcwinex",
                        "desc": "btcwinex - btcwinex Mercantile Exchange"
                    }
                ],
                "symbols_types": [
                    {
                ...
                "supported_resolutions": ["1","5","15","30","60","240","D","5D","W","M"],
    ...
    UDFCompatibleDatafeedBase.prototype._setupWithConfiguration = function (configurationData) {
        ...
        this._configuration.supports_marks = false
        this._configuration.supports_timescale_marks = false
...
var UDFCompatibleDatafeed = /** @class */ (function (_super) {
    ...
    function UDFCompatibleDatafeed(datafeedURL, updateFrequency,market) {
        if (updateFrequency === void 0){
            updateFrequency = 10 * 1000;
        }else{
            updateFrequency = 1 * 1000;
        }
        var _this = this;
        var requester = new Requester();
        var quotesProvider = new QuotesProvider(datafeedURL, requester);
        
        _this = _super.call(this, datafeedURL, quotesProvider, requester, updateFrequency,market) || this;
        return _this;
    }
