...
export default class GetCode extends React.Component{
    ...
    render(){
        const { min, mout } = this;
        return (
            <div className="country_select" onClick={min}>
                <div className="input-group-btn dropdown" >
                    <div className="btn-group">
                        <span style={{border:0}} id="countryText">{this.props.showCode == 0 ? this.state.currentCountry : this.state.currentCode}</span>
                        <i className="iconfont"></i>
                        <span className="iconfont_select" ></span>
                    </div>
                    {
