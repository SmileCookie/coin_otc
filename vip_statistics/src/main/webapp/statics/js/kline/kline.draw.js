var GLOBAL_VAR = {
    ...
    button_down       :  false,
    init			  :  false,
    url				  :  DOMAIN_TRANS+"/markets/klineLastData"
...
/**
 * 计算价格显示区间 如隔：0.005 显示一个价格，页面显示按 0.02， 0.015，0.010显示
 * @returns {Number}
 * @modify by zhanglinbo 20161221
 */
Range.prototype.calcInterval = function() {
    var H = this.getHeight();//获取高度
    var h = this.getMinInterval();//获取最小分多少栏
    if ((H / h) <= 1)
        h = H >> 1;
    var d = this.getRange();//获取当前K线数据最高价-最低价的范围值
    var i = 0;
    while (i > -6 && Math.floor(d) < d) {//位移6位并且向下取整后小于原值
        d *= 10.0;
        i--;
    }
    var m, c;
    for (;; i++) {
        c = Math.pow(10.0, i);//做幂运算 直到toHeight(m)值大于h Math.floor(value * this._ratio + 1.5);
        m = c;
...
{
    this._gradations = [];
    var interval = this.calcInterval();//计算价格分隔区间
    if (interval <= 0.0)
        return;
    //计算最高价格 （最大价格/价格区间）向下取整后再*价格区间
    var v = Math.floor(this.getMaxValue() / interval) * interval;
    do {
        this._gradations.push(v);
        v -= interval;//向下计算显示价格，每个价格减价格区间，直到显示价格小于最低价则退出。小于最低价的价格不显示
...
RangePlotter.prototype.Draw = function(context) {
    ...
    var range = mgr.getRange(rangeName);//获取价格范围对象
    if (range.getRange() == 0.0)
        return;
    var isMainRange = range.getNameObject().getCompAt(2) == "main";
    if (isMainRange) {
    ...
    var gradations = range.getGradations();//获取价格显示数组
    ...
    var gridRects = [];
    for (var n in gradations) {
        var y = range.toY(gradations[n]);//计算价格显示位置
        gridRects.push({x:left, y:y, w:6, h:1});
        gridRects.push({x:right - 6, y:y, w:6, h:1});
        //修改价格保留6位小数 modify by zhanglinbo 20161221
        context.fillText(String.fromFloat(gradations[n], 6), center, y);
    }
    if (gridRects.length > 0) {
        context.fillStyle = theme.getColor(Theme.Color.Grid1);
        Plotter.createRectangles(context, gridRects);
...
ChartSettings.load = function () {
	//优先使用window.localStorage 2016-09-29 by Siven
	if(window.localStorage){
		if (!localStorage["chartSettings"])
	        return;
		var json = localStorage["chartSettings"];
	}else{
...
ChartSettings.save = function () {
	//优先使用window.localStorage 2016-09-29 by Siven
	if(window.localStorage){
		localStorage["chartSettings"] = JSON.stringify(ChartSettings._data);
	}else{
		var exdate = new Date();
		...
		var readyFun = setInterval(function(){
			...
			if(parent.webSocket){
				parent.webSocket.init(function(){
					parent.webSocket.sendMessage(message);
				});
				clearInterval(readyFun);
			}
		},50);
...
function updateKlineData(json){
	var channel = kline.symbol+"_kline_"+GLOBAL_VAR.time_type;
...
function getklinedataWebsocket(){
		...
		if(init){
			...
			if(oldChannel!=channel ){//发送新请求频道，先删除旧渠道
				...
				var message = '{"event":"addChannel","channel":"'+channel+'","lastTime":"'+lastTime+'","isZip":"true"}';
	        	//sendChannel(message); //按时先不用websocket推送 技术
			}
	    	//ajax轮询
		    	
		     if(parent.ajaxRun==false){//ajaxRun webcommon.js 判断是否进行ajax轮询
	    			return;
	    	  }else{
...
function getklinedataAjax(){
     ...
     GLOBAL_VAR.G_HTTP_REQUEST = $.getJSON(GLOBAL_VAR.url+"?jsoncallback=?&"+GLOBAL_VAR.requestParam,function(json) {
		if (GLOBAL_VAR.G_HTTP_REQUEST) {
           if (time != GLOBAL_VAR.time_type || market != GLOBAL_VAR.market_from) {
            ...
            if(!json || !json.datas.data[0]|| json.datas.data[0].length==0){
           	  $(".bk-animationload").fadeOut();
            	return;
            }
            if(!json.isSuc){
	...
	/*
	 GLOBAL_VAR.G_HTTP_REQUEST = $.getJSON({
         type        : "post",
         url         : GLOBAL_VAR.url+"?callback=?",
         dataType    : 'json',
         ...
         success     : function(json) {
             if (GLOBAL_VAR.G_HTTP_REQUEST) {
                 ...
                 if(!json || json.datas.data[0].length==0){
                	  $(".bk-animationload").fadeOut();
                 	return;
                 }
                 if(!json.isSuc){
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    var str = "needTickers=1&symbol=" + mark_from + "&type=" + time_type;
    if (limit != null)
        str += "&size=" + limit;
    else
        str += "&since=" + since;
