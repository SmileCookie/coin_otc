...
Kline.prototype={
		...
		var readyFun = setInterval(function(){
			...
			if(parent.webSocket){
				parent.webSocket.init(function(){
					parent.webSocket.sendMessage('{"event":"addChannel","channel":"'+$this.depthChannel+'","isZip":"'+!JuaBox.isMobile()+'"}');
					parent.webSocket.sendMessage('{"event":"addChannel","channel":"'+$this.tradeChannel+'","isZip":"'+!JuaBox.isMobile()+'"}');
				});
				clearInterval(readyFun);
			}
		},50);
	...
	setFirstRecord:function(callback){
		var $this = this ;
		var currency = $this.symbol;
		console.info(currency);
		
		//1.http 初始化深度
		var depthUrl =   DOMAIN_TRANS +"/depth?callback=?&symbol="+currency+"&lastTime=0&length=10&depth=0" ;
		$.getJSON(depthUrl, function(result){
			if(result){
				var item =  result.datas;
				$this.updateDepth(item["return"]);
			}
    		
    	});
    	
		//2.http 初始化成交记录
		//var jsonUrl =  DOMAIN_TRANS + "/Line/GetTrans-"+marketData[0].market+"?lastTime=0&length=10&depth=0&jsoncallback=?" ;
		var jsonUrl =  DOMAIN_TRANS +"/getLastTrades?callback=?&symbol="+currency+"&last_trade_tid="+$this.last_trade_tid;
	...
	ajaxRedister:function(){
		...
		var $this=this;
		//var jsonUrl =  DOMAIN_TRANS + "/depth-"+marketData[0].market+"?lastTime=0&length=10&depth=0&jsoncallback=?" ;
		var currency = $this.symbol;
		
		var jsonUrl =   DOMAIN_TRANS +"/depth?callback=?&symbol="+currency+"&lastTime=0&length=10&depth=0" ;
    	...
    	var tradeUrl =  DOMAIN_TRANS +"/getLastTrades?callback=?&symbol="+currency+"&last_trade_tid="+$this.last_trade_tid ;
	...
	pushTrades:function(array){
		...
		var that = this;
		if(this.klineTradeInit && array.length>0){
	...
	updateDepth:function(data){
		...
		if(this.lastDepth==null){
			...
			if($buyForm.val() == "" && data.asks.length>0){
				$buyForm.val(data.asks[data.asks.length-1][0]);
			};
			if($sellForm.val() == "" && data.bids.length>0){
	...
	getgasks:function(array){
		if(!array || array.length<=0){//无数据不往下执行 modify by zhanglinbo 20170225
			return [];
		}
