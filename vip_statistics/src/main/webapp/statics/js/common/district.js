function getStrForJson() {
	$("#myJson").text(JSON.stringify(allDists))
}
vip.tips["请选择省"]=["请选择省","Select province"];
vip.tips["请选择市"]=["请选择市","Select city"];
vip.tips["请选择区县"]=["请选择区县","Select district"];
/****
 * 
 * @param {Object} c  ==2 获取市信息  else  初始化区县信息   
 * @param {Object} d      父节点所在的id
 * @param {Object} id
 * @param {Object} ui     是否ui
 */
function getSonType(c, d, id , ui) {
	var a = "";
	var b = "";
	var sign = "";
	if(id)
	...
	if (c == 2) {
		a = "cty";
		b += '<option value="0">-'+vip.L("请选择市")+'-</option>';
		$("#"+sign+"dist").html('<option value="0">-'+vip.L("请选择区县")+'-</option>')
	} else {
		a = "dist";
		b += '<option value="0">-'+vip.L("请选择区县")+'-</option>'
	}
	$.each(allDists, function(e, f) {
		if (f[1] == d) {//id比对
			b += '<option value="' + e + '" >' + f[0] + "</option>"
		}
	});
	$("#" + sign + a).html(b)
	if(ui){
...
function initProvince(id) {
	var a = '<option value="">-'+vip.L("请选择省")+'-</option>';
	$.each(allDists, function(b, c) {
		if (c[1] == "1") {
			a += '<option value="' + b + '" >' + c[0] + "</option>"
		}
	});
...
function getNameById(a, e, b, d) {
	var c = "";
	if (a > 0) {
		$.each(allDists, function(f, g) {
			if (f == a) {
				c += g[0]
			}
		})
...
function initOneDistrictById(a, d, b, id, ui) {
	var c = {
		pro : "",
		city : "",
		dist : ""
	};
	c.pro += '<option value="">-'+vip.L("请选择省")+'-</option>';
	$.each(allDists, function(e, f) {
		if (f[1] == "1") {
			if (a == e) {
				c.pro += '<option value="' + e + '" selected="selected">'
						+ f[0] + "</option>"
			} else {
				c.pro += '<option value="' + e + '" >' + f[0] + "</option>"
			}
		}
	});
	c.city += '<option value="">--'+vip.L("请选择市")+'--</option>';
	$.each(allDists, function(e, f) {
		if (f[1] == a) {
			if (d == e) {
				c.city += '<option value="' + e + '" selected="selected">'
						+ f[0] + "</option>"
			} else {
				c.city += '<option value="' + e + '" >' + f[0] + "</option>"
			}
		}
	});
	c.dist = '<option value="">--'+vip.L("请选择区县")+'--</option>';
	$.each(allDists, function(e, f) {
		if (f[1] == d) {
			if (b == e) {
				c.dist += '<option value="' + e + '" selected="selected">'
						+ f[0] + "</option>"
			} else {
				c.dist += '<option value="' + e + '" >' + f[0] + "</option>"
			}
		}
	});
	...
	$("#"+sign+"pro").html(c.pro);
	$("#"+sign+"cty").html(c.city);
	$("#"+sign+"dist").html(c.dist);
...
function initOneDistrictByName(b, g, d) {
	var e = {
		pro : "",
		city : "",
		dist : ""
	...
	var f;
	var c;
	$.each(allDists, function(h, i) {
		if (i[1] == "1") {
			if (b == i[0]) {
				e.pro += '<option value="' + h + '" selected="selected">'
						+ i[0] + "</option>";
				a = h
			} else {
				e.pro += '<option value="' + h + '" >' + i[0] + "</option>"
			}
		}
	});
	$.each(allDists, function(h, i) {
		if (i[1] == a) {
			if (g == i[0]) {
				e.city += '<option value="' + h + '" selected="selected">'
						+ i[0] + "</option>";
				f = h
			} else {
				e.city += '<option value="' + h + '" >' + i[0] + "</option>"
			}
		}
	});
	$.each(allDists, function(h, i) {
		if (i[1] == f) {
			if (d == i[0]) {
				e.dist += '<option value="' + h + '" selected="selected">'
						+ i[0] + "</option>"
			} else {
				e.dist += '<option value="' + h + '" >' + i[0] + "</option>"
			}
		}
	});
	$("#pro").html(e.pro);
...
var allDists = {
	...
	"460309" : [ "澄迈县", "460300" ],
	"460310" : [ "临高县", "460300" ],
	"460311" : [ "白沙黎族自治县", "460300" ],
	"460312" : [ "昌江黎族自治县", "460300" ],
	"460313" : [ "乐东黎族自治县", "460300" ],
	"460314" : [ "陵水黎族自治县", "460300" ],
	"460315" : [ "保亭黎族苗族自治县", "460300" ],
	"460316" : [ "琼中黎族苗族自治县", "460300" ],
	"460317" : [ "西沙群岛", "460300" ],
	"460318" : [ "南沙群岛", "460300" ],
	"460319" : [ "中沙群岛的岛礁及海域", "460300" ],
	"471004" : [ "高新区", "410300" ],
	"471005" : [ "其它区", "410300" ],
	"500000" : [ "重庆", "1" ],
	"500100" : [ "重庆市", "500000" ],
