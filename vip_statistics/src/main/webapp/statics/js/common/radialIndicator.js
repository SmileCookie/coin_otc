...
;(function ($, window, document) {
    ...
    Indicator.prototype = {
        ...
        value: function (val) {
            ...
            if(((circ) * perVal / 100) + 3*quart == circ){
                
            }
            if(val != 100){
                ctx.beginPath();
                ctx.arc(center, center, indOption.radius, -(quart), ((circ) * perVal / 100) + 3*quart, true);
                ctx.stroke();
            }
            ...
            if (indOption.displayNumber) {
                var cFont = ctx.font.split(' '),
                    ...
                    alignItem = indOption.title ? 'hanging': 'center';
                ...
                ctx.fillText(dispVal, center, 70);
            }
            //add Title
            if(indOption.title) {
                    ctx.fillStyle = '#9A9CA4';
                    ctx.font = "400 13px " + cFont;
                    ctx.textBaseline = 'bottom';
                    ctx.fillText(indOption.title, center,65);
            }
