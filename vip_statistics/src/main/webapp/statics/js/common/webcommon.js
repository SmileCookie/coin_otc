...
function Set2DivHeight(){
	//初始化高度，防止取值错误
//	$(".side_l").css("height","auto");
//	var l_height = $(".side_l").outerHeight();
//	var r_height = $(".side_r").outerHeight();
//	  if (l_height < r_height){
//		  if (r_height > 500)
//		       {$(".side_l").css("height",r_height); } 
//		  else {$(".side_l").css("height",500); }
//	  }
//	      else {$(".side_l").css("height","auto");
//	   }
		 // alert($(".side_l").outerHeight());
}
/***自适应载入miniCss*/
function miniBrand(){
	var noIframe = true;
	if(top.location != self.location){noIframe = false ;}//判断是否是框架页面
	var mini_width = $(window).width();
    if (mini_width < 1200 && noIframe){
		if ($("#mini_css").length == 0){
           // $("head").append("<link rel=stylesheet id='mini_css' type=text/css href='"+vip.staticDomain+"/statics/css/cnbtc2014_mini.css?"+numberID()+"' />");
		}
    }else{
...
$(function() {
	...
	/********************************/
	/**调整并排DIV高度 载入自适应CSS*/
	window.onload = window.onresize = function (){Set2DivHeight();miniBrand();};
	/********************************/
	/**固定二级菜单*/
    var elm = $('.b_menu2');
    if(elm.length > 0){
        ...
        $.event.add(window, "scroll", function() {
            ...
            $(elm).css('z-index',((p) > startPos) ? '9999' : '');
        });
    }
...
/********************************/
/**首页实时行情特效*/
jQuery("#btcMarket").slide({titCell:"none",mainCell:".hd",autoPage:true,effect:"topLoop",easing:"easeOutElastic",mouseOverStop:false,autoPlay:true,interTime:5000,delayTime:1000,startFun:function(){$("#btcMarket").addClass("on");},endFun:function(){$("#btcMarket").removeClass("on");}});
jQuery("#ltcMarket").slide({titCell:"none",mainCell:".hd",autoPage:true,effect:"topLoop",easing:"easeOutElastic",mouseOverStop:false,autoPlay:true,interTime:5000,delayTime:1000,startFun:function(){$("#ltcMarket").addClass("on");},endFun:function(){$("#ltcMarket").removeClass("on");}});
jQuery("#btqMarket").slide({titCell:"none",mainCell:".hd",autoPage:true,effect:"topLoop",easing:"easeOutElastic",mouseOverStop:false,autoPlay:true,interTime:5000,delayTime:1000,startFun:function(){$("#btqMarket").addClass("on");},endFun:function(){$("#btqMarket").removeClass("on");}});
/********************************/
/**关闭提示*/
...
/**资产下拉菜单*/
//SlideBox(".d_assets",".p_drop");
/**语言切换*/
//SlideBox(".b_english",".b_english ul");
/**联系方式*/
//$(".d_kefu").DropSelecter({ControlSeleter:".d_lianxi",Top:0,Left:1,StyleHover:"download_hover"});
//SlideBox(".d_kefu",".d_lianxi");
/**新闻选中
$(".h_xinwen").slide({titCell:".side_l .item"});
*/
/**十星保tab*/
jQuery(".h_10xingbao").slide({mainCell:".bd",titCell:".hd ul li",trigger:"click"});
//交易tab
//jQuery(".b_trading").slide({mainCell:".bd",titCell:".hd ul li a",trigger:"click"});
//交易中心精简版切换
//jQuery(".b_tradtab").slide({mainCell:".bd",titCell:".d_tradtab ul li",trigger:"click",
//   startFun:function(i,c){if (i == 2){$(".d_market").hide();} else {$(".d_market").show();}}
//});
//交易中心高级版切换
//jQuery(".b_tradtab_pro").slide({mainCell:".bd",titCell:".d_tradtab ul li",trigger:"click"});
//
/**幻灯片*/
jQuery(".b_banner").slide({mainCell:".bd ul",effect:"fold",interTime:5000,autoPlay:true});
/**滚动公告*/
//jQuery(".h_notice").slide({mainCell:".bd ul",autoPlay:true,effect:"leftMarquee",interTime:50,vis:5});
/**视频切换*/
jQuery(".b_video").slide({mainCell:".bd ul",effect:"left",autoPlay:true});
/**滚动行情*/
jQuery(".d_hanqing").slide({titCell:".hd ul",mainCell:"ul",autoPage:true,effect:"topLoop",interTime:3000,autoPlay:true});
				...
				if(w>0&&h>0){
				  $("#tinybox").animate({left:endLeft,top:endtop,width:w,height:h},150,'',function(){
					...
					$("#tinybox").animate({top:endtop,height:h},150,'',function(){
						if(h2==99)  p.style.height='auto';
						//$("#fancybox-frame").css("height","100%");
						$("#fancybox-frame").css("height",h-44);
						  if(callback!=null)
				                 callback;
...
function Ask(options){
	var defaults = {
		...
		var content='<div class="AlertMessage"><div class="MessageTitle"><div class="MessageTitle">'+vip.L("WARM_PROMPT")+'</div><div class="MessageCloseC"><a class="MessageClose" onfocus="this.blur()" href="javascript:Close()" title="关闭">Close</a></div></div><div class="MessageHelp"><div class="Message">'+options.Msg+'</div></div><div class="MessageControl"><div class="MessageControl2">'+
	 '<a onfocus="this.blur()" id="nobackButton_1"  class="noback" onclick="'+options.callback2+'" href="javascript:Void();"><h4>'+vip.L("取消")+'</h4></a>'+
	 '<a onfocus="this.blur()" id="okButton_1" class="ok" onclick="'+options.callback+'" href="javascript:Void();"><h4>'+vip.L("确定")+'</h4></a>'+
	 '</div></div></div>';
	NetBox.show(content,550,options.Height,options.CloseTime,options.fromObj);
...
function Ask2(options)
{
	 var defaults = {
	   ...
	   //$(this).bind("click",function(){
			var content='<div class="AlertMessage"><div class="MessageTitle"><div class="MessageTitle">'+vip.L("WARM_PROMPT")+'</div><div class="MessageCloseC"><a class="MessageClose" onfocus="this.blur()" href="javascript:Close()" title="关闭">Close</a></div></div><div class="MessageHelp"><div class="Message">'+options.Msg+'</div></div><div class="MessageControl"><div class="MessageControl2">'+
			'<a onfocus="this.blur()" id="nobackButton_2"  class="noback" onclick="'+options.callback2+'" href="javascript:Void();"><h4>'+vip.L("取消")+'</h4></a>'+
			 '<a onfocus="this.blur()" id="okButton_2" class="ok" onclick="'+options.callback+'" href="javascript:Void();"><h4>'+vip.L("确定")+'</h4></a>'+
		 '</div></div></div>';
		 NetBox.show(content,550,options.Height,options.CloseTime,options.fromObj);
...
//};
//也可以直接  Alert("googf");
function Message(tit,msg,style,options){ 
var defaults = {
     CloseTime:0,//几秒钟后自动关闭
     Msg:'',//信息
     Style:'Alert',//使用的样式
...
if(style) 
   ...
   content+= '<div class="MessageTitle"><div class="MessageTitle">'+options.Tit+'</div><div class="MessageCloseC"><a class="MessageClose" onfocus="this.blur()" href="javascript:Close()" title="关闭">Close</a></div></div>'
   content+='<div class="Message'+options.Style+'"><div class="Message">'+options.Msg+'</div></div>';
   if(options.CloseTime==0)
	   content+='<div class="MessageControl"><div class="MessageControl2">'+
'<a onfocus="this.blur()" id="okButton_2013"  class="ok" onclick="'+options.callback+'" href="javascript:Void();"><h4>'+vip.L('确定')+'</h4></a>'+
'</div></div>';
...
{
  Message(vip.L("WARM_PROMPT"),msg+"","Alert",options);

}
//信息对话框
function Info(msg,options)
{
Message(vip.L("WARM_PROMPT"),msg+"","Info",options);
...
{
Message(vip.L("WARM_PROMPT"),msg+"","Wrong",options);

}
//正确信息对话框
function Right(msg,options)
{
Message(vip.L("WARM_PROMPT"),msg+"","Right",options);

}
//帮助/询问信息对话框
function Help(msg,options)
{
Message(vip.L("WARMPROMPT"),msg+"","Help",options);
...
var defaults = {
     ...
     padding	    : 0,
     IsShow         :false,
     IsCloseOnModal :false,	//是否可以在遮罩层处双击关闭窗口
     fromObj        :null  //从这个地方弹出来,如果是null就不谈
};

options=Object.extend(defaults, options);  
//options.fromObj=T$(options.fromObj); 
	var iframeHeight=(options.height-44)+"px"; 
	var content='';
	if(options.isShowIframeTitle)
	{
	  ...
	  if(options.isIframeAutoHeight)//需要自适应高度 
	  {
		 ...
		 content='<div class="popIframeTitle'+isClose+'"><div class="popIframeTitle">'+options.Title+'</div><div class="popIframeCloseC">'+
		  '<a class="popIframeClose" onfocus="this.blur()" href="javascript:Close()" title="关闭">Close</a></div></div><div class="popIframeContent"><iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0"   hspace="0" ' + ($.browser.msie ? 'allowtransparency="true""' : '') + ' scrolling="' + options.scrolling + '" onload="$(this).height($(this).contents().height());NetBox.resize(0,($(this).contents().height()+44));" src="' + options.Url + '"></iframe></div>';
	  } 
	  else{   
		 content='<div class="popIframeTitle'+isClose+'"><div class="popIframeTitle">'+options.Title+'</div><div class="popIframeCloseC">'+ 
	     '<a class="popIframeClose" onfocus="this.blur()" href="javascript:Close()" title="关闭">Close</a></div></div><div class="popIframeContent"><iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0" hspace="0"  scrolling="' + options.scrolling + '" style="height:'+(options.Height-44)+'px"  src="' + options.Url + '"></iframe></div>';
	  }
	}
	else
	{
		if(options.isIframeAutoHeight)//需要自适应高度
			content='<div class="popIframeContent"><iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + ($.browser.msie ? 'allowtransparency="true""' : '') + '  onload="$(this).height($(this).contents().height());NetBox.resize(0,($(this).contents().height()));" scrolling="' + options.scrolling + '" src="' + options.Url + '"></iframe></div>';		
		else 						
			content='<div class="popIframeContent"><iframe id="fancybox-frame" name="fancybox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + ($.browser.msie ? 'allowtransparency="true""' : '') + ' scrolling="' + options.scrolling + '" src="' + options.Url + '"></iframe></div>';		
	}
	 NetBox.show(content,options.Width,options.Height,0,options.fromObj, options.IsCloseOnModal);
...
jQuery.cookie = function(name, value, options) {
   if (typeof value != 'undefined') { // name and value given, set cookie
	   ...
	   var raw = options.raw ? options.raw : false;
	   if (raw){
	  ...
	  if($.browser.safari){//sofari宽度太短
        ...
        $selecter.mouseover(function(){ 
			...
			if(!$(this).data("dropDownSelected")){ 
				...
				//样式化他 
				$selecter.DropSelecter({ControlSeleter:"#down_"+id,StyleHover:options.StyleHover,StyleSelect:options.StyleDropDown,Left:0,Top:-13,Auto:false,IsShow:true});  
				 if(needScrool){
...
// this.each(function (i) {
 	//母体控件
         ...
         $(window).scroll(function () {
             ...
             var elmtop =$tbl.offset()?$tbl.offset().top:0;  
...
if(!options.IsShow){//清理工作
 ...
 if(options.Position=="top")
	  ...
	  style+="top:"+($(this).offset().top-130)+"px;left:"+($(this).offset().left-getOffsetLeft($(this)))+"px;";  
 }
...
if(options.IsShow)
  {  
	...
	//首先查明当前容器有没有创建过
	$(this).MyTitle({defaultCss:'tipsr',title:vip.L("WARM_PROMPT"),trigger:"manual"});
	if(!$(this).attr("errmsg")) 
	      ...
	      if(arr[i].type.toLowerCase()=="text"||arr[i].type.toLowerCase()=="hidden"||arr[i].type.toLowerCase()=="password"||arr[i].tagName.toLowerCase()=="textarea"){
	    	  if(arr[i].name.length>0){
	    		 if($(arr[i]).attr("valueDemo")!=$(arr[i]).val())
	                str+="&"+encodeURIComponent(arr[i].name)+"="+encodeURIComponent($.trim(arr[i].value));
	    		 else
	    			 str+="&"+encodeURIComponent(arr[i].name)+"= ";
	    	 }
	       if($(arr[i]).is(":visible") && !CheckTextBox($(arr[i]))){ 
...
var vip = {
	...
	mainDomain : GLOBAL.DOMAIN_MAIN,
	vipDomain : GLOBAL.DOMAIN_VIP,
	p2pDomain : GLOBAL.DOMAIN_P2P,
	transDomain : GLOBAL.DOMAIN_TRANS,
	staticDomain : GLOBAL.DOMAIN_STATIC,
	urlsAjax : {},
	/*******
	 * 通用的ajax处理方法  
	...
	ajax : function(ops){//为每个url产生一个对象
		...
		if(curURL == null){
			_this.urlsAjax[rurl] = {loading : false , needLoading : nld , url : rurl}; 
	...
	ajaxDeal : function(ops , curURL){
		    ...
		    $.ajax({
			   ...
			   success:function(xml){
	        	  ...
	        	  }else if(_this.dataType.indexOf('json')==0){
	        		  if(xml.isSuc){
			              if(typeof ops.suc == "function"){
			            	  (function(v){ return ops.suc(v);})(xml)
			              }else{Right(xml.des);}
	...
	tool : {
		...
		initBackBtn : function(){
			var roleId=$.cookie(vip.cookiKeys.rid);
			if(roleId>0){
	...
	user : {///用户相关
		...
		init : function(){
			 ...
			 //$(".b_usermenu .m3").hide();
			 //语言切换
			 //$("#languageSelectA").DropSelecter({ControlSeleter:"#languageSelectCont",Top:0,Left:-5,StyleHover:"download_hover",call : function(){}});
			
			 this.cookieInit = true;
			 this.loginStatus = $.cookie(vip.cookiKeys.uon)=="1";
			 var _this = this;
		 	...
		 	//记录跳转前链接 登陆后返回地址
		 	 ...
		 	 if(JuaBox.isMySelf() && curUrl.indexOf("/u") == -1 && curUrl.indexOf("/self/") == -1){
			   $.cookie(ZNAME+"fromurl",cruLink,{ path: '/' ,domain: document.domain });
			 }
			 if(_this.loginStatus){//登录成功
				    var uname = $.cookie(vip.cookiKeys.uname);
				    $(".logined").show();
					$(".nologin").hide();
					$(".zhuche_btn").hide();//立即注册按钮
					$(".login_btn").css("display","inline-block")
							   	   .html(bitbank.L("返回交易"))
							   	   .attr("myHref",DOMAIN_VIP+"/trade");
				    ...
				    $(".d_assets").show();
				   	$("#assets").show();
					$("#logOut").show();
					$("#assets .u_name").text(this.showTime()+"，"+uname);
				    $(".d_menu1 .m2").show();
				    $("#assets i").attr("class","u-level-" + $.cookie(vip.cookiKeys.vip));
				    _this.balance();
				    _this.zcticker();
					set_menu_hover(2);
					this.assets();
				    ...
				    $(".logined").hide();
					$(".nologin").show();
					$(".zhuche_btn").css("display","inline-block");
					 $(".login_btn").css("display","inline-block")
								    .html(bitbank.L("登录"))
								    .attr("myHref",DOMAIN_VIP+"/login");
		...
		showTime : function(){
			...
			return hours >= 0&&hours < 6 ? vip.L("早上好") : hours >=6&&hours<12? vip.L("上午好"):hours>=12&&hours<13? vip.L("中午好"):hours>=13&&hours<18? vip.L("下午好"): vip.L("晚上好");
		},
		firstTitle : "",
		lastP : [0 , 0],
		upOrDown : function(index , id ,jsons , type){
			if(index != 0)return;
			if(this.lastP[id] == 0){
				this.lastP[id] = jsons[0];
			}else{
				if(jsons[0] > this.lastP[id]){//涨
					$("#statisticsDiv li." + type).addClass("up").removeClass("down");
				}else if(jsons[0] < this.lastP[id]){
					$("#statisticsDiv li." + type).addClass("down").removeClass("up");
				}
				this.lastP[id] = jsons[0];
			}
		},
		...
		ticker_deleted : function(){
			var _this = this;
			$.getJSON(vip.transDomain + "/line/topall?jsoncallback=?",  function(json) {
				$("#statisticsDiv li.btc").each(function(){
					$(this).find("em").each(function(i){
						$(this).text(json[0].btcdefault_hotdata[i]);
						_this.upOrDown(i , 0 , json[0].btcdefault_hotdata , "btc");
					});
				});
				$("#statisticsDiv li.ltc").each(function(){
					$(this).find("em").each(function(i){
						$(this).text(json[0].ltcdefault_hotdata[i]);
						_this.upOrDown(i , 1 , json[0].ltcdefault_hotdata , "btc");
					});
				});
				//////首页的动态显示
				if($("#btcMarket").length == 1){
					$("#btcMarket .hd b").text(CurrencyFormatted(json[0].btcdefault_hotdata[0]));
					$("#btcMarket .bd span.m1 b").text(CurrencyFormatted(json[0].btcdefault_hotdata[1]));
					$("#btcMarket .bd span.m2 b").text(CurrencyFormatted(json[0].btcdefault_hotdata[2]));
					$("#ltcMarket .hd b").text(CurrencyFormatted(json[0].ltcdefault_hotdata[0]));
					$("#ltcMarket .bd span.m1 b").text(CurrencyFormatted(json[0].ltcdefault_hotdata[1]));
					$("#ltcMarket .bd span.m2 b").text(CurrencyFormatted(json[0].ltcdefault_hotdata[2]));
					$("#btqMarket .hd b").text(json[0].btqdefault_hotdata[0]);
					$("#btqMarket .bd span.m1 b").text(json[0].btqdefault_hotdata[1]);
					$("#btqMarket .bd span.m2 b").text(json[0].btqdefault_hotdata[2]);
				}
				//标题显示 
				//document.title ="BTC:￥" + CurrencyFormatted(json[0].btcdefault_hotdata[0]) +"-"+ $title  ;
			});
		},
		uticker : function(){
			if(this.isLogin()){
				var _this = this;
				$.getJSON(vip.vipDomain + "/user/userticker?callback=?",  function(json) {
					var indexUserFundsSpan = $("#indexUserFunds p span");
					if(indexUserFundsSpan.length == 4){indexUserFundsSpan.eq(0).text(json.total)}
					$("#finaPanelDown1 .rmb .d1 b").html(json.total);
				});
			}
		},
		...
		zcticker : function(){},
		zcticker_deleted : function(){//总资金的方法
			$.getJSON(vip.vipDomain + "/user/zcticker?callback=?",  function(json) {
				var funds = json.assets;
				$("#finaPanelDown1 .rmb b").each(function(i){
					$(this).text(funds[i]);
				});
				$("#M_userDrop1 .assetNum").each(function(i){
					$(this).text(funds[i]);
				});
				if($("#myTotalBalance").length > 0){//首页总资产
					$("#myTotalBalance").text(funds[0]);
				}
			});
		},
		assets : function(){///获取用户资金信息
			var _this = this;
			//SlideBox(".d_assets",".p_drop");
			var w_drop = $("#finaPanelDown1").width();
			var ccLength=parseInt($("#finaPanelDown1").width())-w_drop;
			//$("#finaPanelDown1").DropSelecter({ControlSeleter:"#finaPanelDown2",Top:0,Left:ccLength,StyleHover:"download_hover",call : function(){
			//	_this.balance();
			//}});
		},
		resetUserFunds : function(){
			var _this = this;
			$.getJSON(vip.vipDomain + "/u/resetUserFunds?callback=?",  function(result) {
				if(result.isSuc){
					Right("资金刷新成功", {callback:"Close()"});
					_this.balance();
				}else{
					Info(result.des, {callback:"Close()"});
				}
			});
		},
		balance : function(){},
		balance_deleted : function(){
			...
			$.getJSON(vip.vipDomain + "/u/getBalance?callback=?",  function(result) {
				...
				if($("#M_userDrop2").length > 0){
					$("#M_userDrop2 .assetNum").each(function(i){
						if (i < 18) {
							$(this).text(funds[i]);
						}
					});
					//显示比特权数据
					if(funds[9] != 0 || funds[10] != 0 || funds[11] != 0){
						$(".btqDisplay").show();
					};
					$("#M_userDrop2 .daiNum").eq(0).text(funds[12])
					$("#M_userDrop2 .daiNum").eq(1).text(funds[13])
					$("#M_userDrop2 .daiNum").eq(2).text(funds[14])
					var hasLoan = funds[15] == 1 ? vip.L("借入") : vip.L("借出");
					$("#M_userDrop2 .daiTit").html(hasLoan);
					
					var ethRow = $("#M_userDrop2").find('tr[data-id="ethRow"]');
					ethRow.find('span[data-id="total"]').text(funds[19]);
					ethRow.find('span[data-id="usable"]').text(funds[20]);
				}
				...
				if($(".b_tradinfo .bd em").length > 0){
					...
					$(".b_tradinfo h3").text(result.funds[16]);
				}
				
				var indexUserFundsSpan = $("#indexUserFunds p span");
				if(indexUserFundsSpan.length == 4){
					indexUserFundsSpan.each(function(i){
						if(i > 0){//3i - 1
							$(this).text(funds[3*(i-1)]);
						}else{
							$(this).text(funds[16]);
						}
					});
				...
				if(funds[12] > 0 || funds[13]>0 || funds[14]>0 || openP2p){
					$(".p2pLoanOut").show();
				}else{
					$(".p2pLoanOut").hide();
				}
		...
		login : function(call,isShowTitle,nce){
			...
			Iframe({Url: vip.vipDomain + '/user/loginAuthentication?callback='+callBack+'&needClose='+nce,isShowIframeTitle:ist, fromObj:T$("login"),Width:550,Height:350,scrolling:'no',	isShowClose:false,Title:'用户登录',isShowIframeTitle:true});
		},
	...
	form : {error : ""},
	getLan : function(){
		var lan=$.cookie(vip.cookiKeys.lan);
		if(lan == "en"){
			return "en";
		}
		return "cn";
	...
	L : function(key , reps){
		try {
			...
			if(reps && reps.length > 0){
				for(var o in reps){
					ks = ks.replace(reps[o].k , reps[o].v)
			    }
			}
			return ks;
		} catch (e) {
	...
	,"WARM_PROMPT" : ["WARM_PROMPT" , "prompts you"]
    ...
    setLan : function(lan){
    	 $.getJSON(vip.mainDomain + "/setlan?callback=?&lan="+lan,  function(result) {
    		 if(result.isSuc){
    			 location.reload();
    		 }
    	 });
	},
	setCurrency:function(mon){
		console.log(mon)
		$.getJSON(vip.mainDomain + "/setCurrency?callback=?&currency="+mon,  function(result) {
			if(result.isSuc){
				location.reload();
			}
		});
...
vip.list = {
	...
	del : function(ops){///del  delete缩写  实现当前模块的删除功能   ops={id }
		...
		var id = ops.id,
			_this = this;
		Ask2({Title:vip.L("确定删除本项吗？"),call:function(){
	...
	ajaxPage : function(ops , e){
		...
		},dataType : "text",div : div,timeout : to});
		if(e){
			if ( e && e.preventDefault ) 
			e.preventDefault(); 
			// A shortcut for stoping the browser action in IE 
...
vip.p2p = {
	 ...
	 * 还款公用方法
	 */	
	repay:function(id,pageNo){
		JuaBox.frame("/manage/loan/repay/repay?id=" + id + "&pageNo=" + pageNo , {
			width : 400,
		});
	},
	...
	/**
	 * 
	 * @param obj 输入控件对象this
	 * @param max 最大小数位数 2
	 * @param type 无用
	 */
	xswCheck : function(obj, max, type) {// 小数位验证
		var m = max + 1;
		var iv = $(obj).val();
	 ...
	 * @param {Object} ops.div 显示loading效果的DIV ID
	 * 自动委托提交
	 * 填写每个币种的自动委托，填0将不委托，可用资金超过最低额将进行委托
	 */
	doAutoOut : function() {
		if (!vip.user.checkLogin()) {
			return;
		}
		var formId = "autop2ps";//auto里面的第一个div  id
	 ...
	 * @param pageNo 当前页码
	 */
	doCancel : function(loanId, pageNo) {
		var _this = this;
		if (!vip.user.checkLogin()) {
			return;
		}
		
		JuaBox.info(bitbank.L("您确定要取消该投资信息吗？"), {
			btnFun1 : function() {
	 ...
	 * @param pageNo 页码
	 */
	contShow : function(i, pageNo) {
		var curCon = $(".lo-record .into_box" + (i + 1));
		// alert("curCon:" + curCon.attr("rel") + ",index:" + i + "," +
		// curCon.length + "," + curCon.attr("id"));
		$(".bk-tabList-bd .con").hide();
		curCon.show();
		if (curCon.attr("rel")) {
			var rel = curCon.attr("rel");
			if (pageNo) {
				//分页删除问题， 删除后要进行判断路径的 ？ and & ，带问号才是正确的跳转，
				if (pageNo > 0) {
					//rel 的后面有？带参数则用&， 反之加上？。 
					if (rel.indexOf("?") > -1) {
						rel += "&page=" + pageNo;
			...
			$(".lo-record .con").removeAttr("id");
			curCon.attr("id", "shopslist" + i);
			vip.list.defaultDiv = "shopslist" + i;
			vip.list.ajaxPage({
				url : rel,
				div : "shopslist" + i,
				timeout : 5000,
	 ...
	 * @param isIn 是否借入
	 */
	doTrans : function(id, isIn) {
		if (!vip.user.checkLogin('(function(){vip.p2p.doTrans(' + id
				+ ')})()')) {
			return;
		}
		var tit = isIn ? "确认投资信息" : "确认借款信息";
	
		JuaBox.frame("/manage/loan/record/loan-" + id + "&pageNo=" + pageNo , {
			width : 560,
			height : isIn ? 430 : 560,
			scrolling : 'no',
